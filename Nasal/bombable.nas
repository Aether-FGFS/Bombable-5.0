...
# allow dogfighting:
#
# 1. BOMBABLE: Makes objects bombable.  They will detect hits, change livery
#    according to damage, and finally start on fire and smoke when sufficiently
#    damaged. There is also a function to  change the livery colors according to
#    damage level.
#
# Bombable also works for multiplayer objects and allows multiplayer dogfighting.
#
# In addition, it creates an explosion whenever the main aircraft hits the ground
# and crashes.
#
# 2. GROUND: Makes objects stay at ground level, adjusting pitch to match any slope
#    they are on. So, for instance, cars, trucks, or tanks can move and always stay
#    on the ground, drive up slopes  somewhat realistically, etc. Ships can be placed
#    in any lake and will automatically find their  correct altitude, etc.
#
# 3. LOCATE: Usually AI objects return to their initial start positions when
#    FG re-inits (ie, file/reset).
#    This function saves and maintains their previous position prior to the reset
#
# 4. ATTACK: Makes AI Aircraft (and conceivable, other AI objects as well) swarm
#    and attack the main aircraft
#
...
#
# Or two (or more) players can choose aircraft set up for dogfighting (see
# readme in accompanying documentation) and dogfight via multiplayer.  Damage,
# smoke, fire, and explosions are all transmitted via multiplayer channels.
#
# Notes:
...
var debprint = func {
  ...
  if ( getprop(""~bomb_menu_pp~"debug") ) {
    ...
    foreach (var elem;arg) {
      if ( elem != nil ) {
        if ( typeof(elem) == "scalar") {
          outputs = string.trim(outputs) ~ " " ~ elem;
        } else {
          debug.dump(elem);
        }
      }
    ...
    if ( typeof(call3) == "vector" ) {
      outputs = outputs ~ call3["3"] ~ " ";
    }
    if ( typeof(call4) == "vector" ) {
      outputs = outputs ~ call4["3"] ~ " ";
    }
    outputs=outputs ~ ")";
...
var check_overall_initialized = func(nodeName) {
  ...
  # joysticks & the like
  var init_allowed = 0;
  if ( find("/ai/models/", nodeName ) != -1 ) {
    init_allowed = 1;
  }
  if ( find("/multiplayer/", nodeName ) != -1 ) {
    init_allowed = 1;
  }
  ...
  if ( init_allowed != 1 ) {
    ...
    # 1 means abort; it's initialized already or can't/shouldn't be initialized,
    return 1;
  }
...
var mpprocesssendqueue = func {
  ...
  # This was experimental: mpTimeDelaySend * (97.48 + rand()/20 )
  settimer(func {
    mpprocesssendqueue();
  }, mpTimeDelaySend );
...
var mpsend = func (msg) {
  # adding systime to the end of the message ensures that each message is
  # unique--otherwise messages that happen to be the same twice in a row will
...
var put_ballistic_model = func(myNodeName="/ai/models/aircraft", path="AI/Aircraft/Fire-Particles/fast-particles.xml") {
  ...
  # if ( (myNodeName == "/environment") or (myNodeName == "environment") ) {
  #   myNodeName = "";
  # }
...
var put_remove_model = func(lat_deg=nil, lon_deg=nil, elev_m=nil, time_sec=nil, startSize_m=nil, endSize_m=1, path="AI/Aircraft/Fire-Particles/flack-impact.xml" ) {
  ...
  settimer( func {
    ...
    if ( startSize_m != nil ) {
      setprop("/bombable/fire-particles/flack-startsize", startSize_m);
    }
    if ( endSize_m != nil ) {
      setprop("/bombable/fire-particles/flack-endsize", endSize_m);
    }
...
var start_terrain_fire = func ( lat_deg, lon_deg, alt_m=0, ballisticMass_lb=1.2 ) {
  ...
  if ( info != nil ) {
    ...
    if ( (typeof(info[1]) == "hash") and contains(info[1], "solid") and (info[1].solid == 0) ) {
      return;
    } else {
      debprint(info);
    }
  ...
  if ( (ballisticMass_lb == nil) or (ballisticMass_lb < 0) ) {
    ballisticMass_lb = 1.2;
  }
  if ( ballisticMass_lb < 3 ) {
    time_sec = 20;
    fp="AI/Aircraft/Fire-Particles/fire-particles-very-very-small.xml";
  } elsif ( ballisticMass_lb < 20 ) {
    time_sec = 60;
    fp="AI/Aircraft/Fire-Particles/fire-particles-very-very-small.xml";
  } elsif ( ballisticMass_lb < 50 ) {
    time_sec = 120;
    fp="AI/Aircraft/Fire-Particles/fire-particles-very-small.xml";
  } elsif ( ballisticMass_lb > 450 ) {
    time_sec = 600;
    fp="AI/Aircraft/Fire-Particles/fire-particles.xml";
  } elsif ( ballisticMass_lb > 1000 ) {
    time_sec = 900;
    fp="AI/Aircraft/Fire-Particles/fire-particles-large.xml";
  } else {
    time_sec = 300;
    fp="AI/Aircraft/Fire-Particles/fire-particles-small.xml";
  }
  ...
  if ( ballisticMass_lb >= 1000 ) {
    put_remove_model(lat_deg:lat_deg, lon_deg:lon_deg, elev_m:alt_m+1, time_sec:time_sec*.9, startSize_m: nil, endSize_m:nil, path:fp );
  }
  if ( ballisticMass_lb >= 1500 ) {
    put_remove_model(lat_deg:lat_deg, lon_deg:lon_deg, elev_m:alt_m+2, time_sec:time_sec*.8, startSize_m: nil, endSize_m:nil, path:fp );
  }
  if ( ballisticMass_lb >= 2000 ) {
    put_remove_model(lat_deg:lat_deg, lon_deg:lon_deg, elev_m:alt_m+3, time_sec:time_sec*.7, startSize_m: nil, endSize_m:nil, path:fp );
  }
  ...
  if ( ballisticMass_lb > 50 ) {
    ...
    settimer(func {
      put_remove_model(lat_deg:lat_deg, lon_deg:lon_deg, elev_m:alt_m, time_sec:time_sec2, startSize_m: nil, endSize_m:nil, path:fp2 );
    } , time_sec);

    time_sec3=120; fp3="AI/Aircraft/Fire-Particles/fire-particles-very-very-small.xml";
    settimer(func {
      put_remove_model(lat_deg:lat_deg, lon_deg:lon_deg, elev_m:alt_m, time_sec:time_sec3, startSize_m: nil, endSize_m:nil, path:fp3 );
    } , time_sec+time_sec2);

    time_sec4=120; fp4="AI/Aircraft/Fire-Particles/fire-particles-very-very-very-small.xml";
    settimer(func {
      put_remove_model(lat_deg:lat_deg, lon_deg:lon_deg, elev_m:alt_m, time_sec:time_sec4, startSize_m: nil, endSize_m:nil, path:fp4 );
    } , time_sec+time_sec2+time_sec3);
  }
}
...
var put_tied_weapon = func(myNodeName="", elem="", startSize_m=.07, endSize_m=.05, path="AI/Aircraft/Fire-Particles/Fire-Particles.xml ") {
  ...
  if ( startSize_m != nil ) {
    setprop("/bombable/fire-particles/projectile-startsize", startSize_m);
  }
  if ( endSize_m != nil ) {
    setprop("/bombable/fire-particles/projectile-endsize", endSize_m);
  }
...
var speedDamage = func {
  ...
  if ( !damage_enabled and !warning_enabled ) {
    return;
  }

  var currSpeed_kt = getprop("/velocities/airspeed-kt");
  if ( (currSpeed_kt == 0) or (currSpeed_kt == nil) ) {
    return;
  }
  ...
  if ( (speedDamageThreshold_kt == 0) or (speedDamageThreshold_kt == nil) ) {
    speedDamageThreshold_kt=7000;
  }
  if ( (speedWarningThreshold_kt == 0) or (speedWarningThreshold_kt == nil) ) {
    speedWarningThreshold_kt=7000;
  }

  var speedDamageMultiplier_PercentPerSecond = getprop(""~vulnerabilities_pp~"airspeed_damage/damage_multiplier_percentpersecond/");

  if ( speedDamageMultiplier_PercentPerSecond == nil ) {
    speedDamageMultiplier_PercentPerSecond=1;
  }
  ...
  if ( damage_enabled and currSpeed_kt > speedDamageThreshold_kt ) {
    mainAC_add_damage( speedDamageMultiplier_PercentPerSecond * (currSpeed_kt -speedDamageThreshold_kt)/100, 0, "speed", "" ~ round( currSpeed_kt ) ~ " kts (overspeed) damaged airframe!" );
  }
}
####################################################
...
var accelerationDamage = func {
  ...
  if ( (currAccel_g == 0) or (currAccel_g == nil) ) {
    return;
  }
  ...
  if ( (accelDamageThreshold_g == 0) or (accelDamageThreshold_g == nil) ) {
    accelDamageThreshold_g    = 50;
  }
  if ( (accelWarningThreshold_g == 0) or (accelWarningThreshold_g == nil) ) {
    accelWarningThreshold_g   = 10;
  }

  var accelDamageMultiplier_PercentPerSecond = getprop(""~GF_damage_pp~"damage_multiplier_percentpersecond/"~a);

  if ( accelDamageMultiplier_PercentPerSecond == nil ) {
    accelDamageMultiplier_PercentPerSecond = 8;
  }
  ...
  if ( damage_enabled and (currAccel_g > accelDamageThreshold_g) ) {
    mainAC_add_damage( accelDamageMultiplier_PercentPerSecond * (currAccel_g -accelDamageThreshold_g)/100, 0, "gforce", "" ~ sprintf("%1.2f", currAccel_g ) ~ "g force damaged airframe!" );
  }
}
...
# WW2 aircraft sometimes has higher max G, but it is interesting because pilot
# did not have G-suit, and trained pilots could not resist 5g for more than some seconds without G-suit.
# the strongest aircraft of WWII were the Italian monoplane fighters. They were built to
# withstand 8g normal load with 12g failure load. The same spec for German aircraft was
# 6g - 8.33g. For the late war P51s it was 5.33g
...
#  FIAT G.50 had an ultimate factor of 14 g. According to Dottore Eng. Gianni Cattaneo´s Profile
# booklet on the Macchi C.202, it had an ultimate factor of no less than 15.8 g! That would make
# it virtually indestructible. Also the Hawker Tempest was strong with its 14+ G strength.
# http://www.aviastar.org/air/japan/mitsubishi_a6m.php :
# Most Japanese fighters were designed to withstand a force of 7g. From 1932 all Japanese warplanes
# were required to meet a safety load factor of 1.8 so the limit for the A6M had to be 12.6g (1.8x7g).
#
...
var setAttributes = func (attsObject=nil) {
  ...
  if ( attsObject == nil ) {
    attsObject = {
      ...
      vulnerabilities: {
        ...
        fireDamageRate_percentpersecond      : 0.4,
  ...
  if (string.match(aircraftname,"A6M2*" )) {
    ...
    attsObject = {
      ...
      #
      # All dimensions are in meters
      # source: https://www.wikiwand.com/en/articles/Mitsubishi_A6M_Zero
      #
      dimensions : {
        width_m                              : 17.53,
        length_m                             : 16.26,
    ...
    attsObject = {
      ...
      #
      # All dimensions are in meters
      # source: https://www.wikiwand.com/en/articles/Fairchild_Republic_A-10_Thunderbolt_II
      #
      dimensions : {
        width_m                              : 17.53,
        length_m                             : 16.26,
      ...
      vulnerabilities: {
        ...
        gforce_damage: {
          ...
          damage_multiplier_percentpersecond : {positive:3, negative:3 }
        },
        redout: {
          enabled                            : 1,
          parameters: {
            blackout_onset_g                 : 7,
            blackout_complete_g              : 12,
            redout_onset_g                   : -2,
            redout_complete_g                : -5
          }
        }
    ...
    attsObject = {
      ...
      #
      # All dimensions are in meters
      # source: https://www.wikiwand.com/en/articles/Sopwith_Camel
      #
      dimensions : {
        width_m                              : 17.53,
        length_m                             : 16.26,
    ...
    attsObject = {
      ...
      #
      # All dimensions are in meters
      # source: https://www.wikiwand.com/en/articles/Fokker_Dr.I
      #
      dimensions : {
        width_m                              : 17.53,
        length_m                             : 16.26,
  ...
  if ( attsSet == 0 ) {
    setlistener("/sim/signals/reinit", func {setAttributes(attsObject)} );
...
var startFire = func (myNodeName="", model="") {
  ...
  if ( (model == nil) or (model == "") ) {
    model="AI/Aircraft/Fire-Particles/fire-particles.xml";
  }
  var fireNode = put_tied_model(myNodeName, model);

  # if ( myNodeName != "" ) {
  #   type=props.globals.getNode(myNodeName).getName();
  # } else {
  #   type="";
  # }
  # if ( type == "multiplayer" ) {
  #   mp_send_damage(myNodeName, 0);
  # }
  ...
  if ( (burnTime == 0) or (burnTime == nil) ) {
    burnTime = 1800;
  }
  settimer(func {
    deleteFire(myNodeName,fireNode);
  }, burnTime);
  ...
  # we usually start with the name & then use props.globals.getNode(nodeName) to get the node object if necessary.
  return fireNodeName;
...
var startSmoke = func (smokeType, myNodeName="", model="") {
  ...
  if ( (model == nil) or (model =="") ) {
    model = "AI/Aircraft/Fire-Particles/"~smokeType~"-particles.xml";
  }
  var fireNode = put_tied_model(myNodeName, model);
  ...
  if ( burnTime >= 0 ) {
    settimer(func {
      deleteSmoke(smokeType, myNodeName,fireNode);
    }, burnTime);
  }
  ...
  fireNodeName="/models/" ~ fireNode.getName() ~ "[" ~ fireNode.getIndex() ~ "]";
   ...
   # we usually pass around the name & then use props.globals.getNode(nodeName) to get the node object if necessary.
   return fireNodeName;
}
...
var revitalizeAllAIObjects = func (revitType="aircraft", preservePosSpeed=0) {
  ...
  if ( (typeof(min_dist_km) == "nil") or (min_dist_km == "") or (min_dist_km == 0) ) {
    min_dist_km = 1;
  }

  var max_dist_km = getprop(bomb_menu_pp~"dispersal-dist-max_km");
  if ( (typeof(max_dist_km) == "nil") or (max_dist_km == "") or (max_dist_km == 0) or (max_dist_km < min_dist_km) ) {
    max_dist_km = 16;
  }
  ...
  foreach (elem;ai) {
    ...
    if ( type == "aircraft" ) {
      if ( preservePosSpeed == 1 ) {
        ...
        if ( max_alt_ft < 2*min_alt_ft ) {
          max_alt_ft = 2*min_alt_ft;
        }
        if ( max_alt_ft < 10000 ) {
          max_alt_ft = 16000;
        }
    ...
    if ( (preservePosSpeed == 0) or (currSpeed_kt == 0) ) {
      ...
      # defaults
      if ( type == "aircraft" ) {
        if ( (min_vel_kt == nil) or (min_vel_kt < 1) ) {
          min_vel_kt = 50;
        }
        ...
        if ( (attack_vel_kt == nil) or (attack_vel_kt <= cruise_vel_kt) ) {
          attack_vel_kt = 1.5*cruise_vel_kt;
        }
        if ( (max_vel_kt == nil) or (max_vel_kt <= attack_vel_kt) ) {
          max_vel_kt = 1.5*attack_vel_kt;
        }
      } else {
        if ( (min_vel_kt == nil) or (min_vel_kt < 1) ) {
          min_vel_kt = 10;
      ...
      if ( vel < min_vel_kt or vel == 0 ) {
        vel = (attack_vel_kt-cruise_vel_kt)*rand() + cruise_vel_kt;
      }
      if ( vel > max_vel_kt) {
        vel = max_vel_kt;
      }
...
var resetBombableDamageFuelWeapons = func (myNodeName) {
  # if ( (myNodeName == "") or (myNodeName == "environment") ) {
  #   myNodeName = "/environment";
  # }
  ...
  if ( props.globals.getNode( ""~myNodeName~"/bombable" ) == nil ) {
    return;
  }
...
var init_bombable_dialog = func () {
  ...
  if ( (bomb_menuNum == nil) or (bomb_menuNum == -1) ) {
    ...
    for (var i=0;i<300;i+=1) {
      ...
      if ( typeof(p) == "nil" ) {
        ...
        # aha, we've already set up the menu once before.  So just re-use it. This happens in FG 2016.x etc when the user re-inits.
        if ( (typeof(l) != "nil") and (l == "Bombable") ) {
          bomb_menuNum = i;
  ...
  # must be unique name from all others in the menubar or they both pop up together
  props.globals.getNode("/sim/menubar/default/menu["~bomb_menuNum~"]/item/label", 1).setValue("Bombable Options");
  ...
  # must be unique name from all others in the menubar or they both pop up together
  props.globals.getNode("/sim/menubar/default/menu["~bomb_menuNum~"]/item[1]/label", 1).setValue("Bombable Statistics");
...
var targetStatusPopupTip = func (label, delay = 5, override = nil) {
  ...
  var tmpl = props.Node.new({ name   : "PopTipTarget",
                              modal  : 0,
                              layout : "hbox",
                              y      : 70,
  ...
  settimer(func {
    if ( currTimerTarget == thisTimerTarget) {
      popdown(tipArgTarget);
    }
  }, delay, 1);
}
...
var selfStatusPopupTip = func (label, delay = 10, override = nil) {
  # gui prob
  # return;
  var tmpl = props.Node.new({ name   : "PopTipSelf",
                              modal  : 0,
                              layout : "hbox",
                              y      : 140,
                              text   : { label : label, padding : 6 }
  ...
  # Final argument is a flag to use "real" time, not simulated time
  settimer(func {
    if ( currTimerSelf == thisTimerSelf ) {
      popdown(tipArgSelf);
    }
  }, delay, 1);
}
...
var dialog = {
  ...
  create : func {
    ...
    # settimer so there is time for the distance values to be put on the prop. tree
    lrevitDISAir.prop().getNode("binding[0]/script", 1).setValue("settimer( func {bombable.revitalizeAllAIObjects(\"aircraft\",2); }, 1);");
...
var init_bombable_dialog_listeners = func {
  ...
  setlistener(""~bomb_menu_pp~"main-weapon-realism-combo", func {
    ...
    if ( val == "Ultra-realistic" ) {
      ...
      # value "Easier" is the default
      setprop(bomb_menu_pp~"easy-mode", 0);
      setprop(bomb_menu_pp~"super-easy-mode", 1);
    }

  # 0,0 means (1) do on initial startup and (1) call listener func only when value is changed
  },1,1);
  ...
  setlistener(""~bomb_menu_pp~"ai-weapon-power-combo", func {
    ...
    if ( val == "More effective" ) {
      ...
      # value "Much less effective" is the default
  ...
  setlistener(""~bomb_menu_pp~"ai-aircraft-skill-combo", func {
    ...
    if ( val == "Very skilled" ) {
      ...
      # value "Unskilled" is the default
...
var setupBombableMenu = func {
  ...
  foreach (var smokeType; [["fire",88, 3600],
                           ["jetcontrail", 77, -1],
                           ["smoketrail", 55, -1],
                           ["pistonexhaust", 15, -1],
                           ["damagedengine",  55, -1],
...
var damage_msg = func (callsign, damageAdd, damageTotal, smoke=0, fire=0, messageType=1) {
  ...
  if ( (masterLock == nil) or (masterLock == "") ) {
    masterLock = 0;
  }
...
var reset_msg = func () {
  ...
  return sprintf ("%6s", callsign) ~ Binary.encodeByte(2);
}
...
var parse_msg = func (source, msg) {
  ...
  # update of callsign's current damage, smoke, fire situation
    ...
    # ai_loc = "/ai/models";
    # var mp_aircraft = props.globals.getNode(ai_loc).getChildren("multiplayer");
    # mp is the node of a multiplayer AI aircraft
    # foreach (mp;mp_aircraft) {
...
var fire_loop = func(id, myNodeName="") {
  ...
  settimer(func {
    fire_loop(id, myNodeName);
  }, fireLoopUpdateTime_sec - 0.5 + rand());
  ...
  if (getprop(""~myNodeName~"/bombable/fire-particles/fire-burning")) {
    ...
    if (rand()<.02/fireLoopUpdateTime_sec)  {
      # turn the big explosion off quickly so it only affects a few of the fires for a moment--they put out smoke particles 4X/second
      settimer(func {
    ...
    if (damageRate_percentpersecond==nil) {
      damageRate_percentpersecond = 0;
    }
    if (damageRate_percentpersecond==0) {
      damageRate_percentpersecond = 0.1;
    }
    ...
    if (myNodeName=="") {
      ...
      if (type != "multiplayer") {
        add_damage( damageRate_percentpersecond/100 * fireLoopUpdateTime_sec , myNodeName,"nonweapon" );
      }
    }
  }
}
...
var hitground_stop_explode = func (myNodeName, alt) {
  ...
  # this affects the slow-down system which is handled by add-damage, and will stop any forward movement very quickly
  setprop(""~myNodeName~"/bombable/on-ground",  1 );

  # and once we have buried ourselves in the ground we are surely dead; this also will stop any & all forward movement
  ...
  if ( exploded==nil or !exploded ) {
    ...
    if (vuls.explosiveMass_kg<0) {
      vuls.explosiveMass_kg=1;
    }
...
var addAltitude_ft = func  (myNodeName, altAdd_ft=40 , time=1 ) {
  ...
  # if (elapsed==0) {
  #   setprop(""~myNodeName~"/position/addAltitude_starting_alt_ft", currAlt_ft );
  # } else {
  #   var startAlt_ft = getprop(""~myNodeName~"/position/addAltitude_starting_alt_ft");
  # }
  ...
  if (elapsed < time) {
    settimer(func {
      addAltitude_ft (myNodeName,altAdd_ft,time);
    }, loopTime);
  } else {
    setprop(""~myNodeName~"/position/addAltitude_elapsed", 0 );
  }
}
...
var ground_loop = func( id, myNodeName ) {
  ...
  settimer(func {
    ground_loop(id, myNodeName);
  }, (0.5 + rand())*updateTime_s );

  #Allow this function to be disabled via menu/it can kill framerate at times
  if (! getprop( bomb_menu_pp~"ai-ground-loop-enabled") or ! getprop(bomb_menu_pp~"bombable-enabled") ) {
    return;
  }
  ...
  if (onGround) {
    # never need thorough when crashed
    thorough=0;
  }
  ...
  if (type=="aircraft" and damageValue < 0.95 and (currAlt_ft - toFrontAlt_ft) > 3* alts.minimumAGL_ft) {
    return;
  }
  ...
  if (type=="aircraft" and ! onGround ) {
    ...
    #  4. current altitude
    if ( radarAheadAlt_ft > toFrontAlt_ft and  (damageValue < 0.8 ) and (radarAheadAlt_ft + alts.minimumAGL_ft > currAlt_ft )) {
  ...
  if (thorough or ( damageValue > 0.8 and ! onGround ) ) {
    # must convert this from radians to degrees, thus the 180/pi
    pitchangle1_deg = rad2degrees * math.atan2(toFrontAlt_ft - toRearAlt_ft, dims.length_ft + 2* FGAltObjectPerimeterBuffer_ft );
    ...
    # in feet
    toRightAlt_ft=elev (GeoCoord2.lat(), GeoCoord2.lon()  );
    GeoCoord2.apply_course_distance(heading-90, dims.width_m + 2*FGAltObjectPerimeterBuffer_m );
    # in feet
    toLeftAlt_ft=elev (GeoCoord2.lat(), GeoCoord2.lon()  );
    # must convert this from radians to degrees, thus the 180/pi
    rollangle_deg = 90 - rad2degrees * math.atan2(dims.width_ft + 2 * FGAltObjectPerimeterBuffer_ft, toLeftAlt_ft - toRightAlt_ft );
  ...
  # This is to get the object to "full crashed position", ie, on the
  # ground for an aircraft, fully sunk for a ship, etc.
  ...
  if ( damageValue > 0.8  )  {
    ...
    # debprint( "alt=", alt_ft, " currAlt_ft=",currAlt_ft,
    #            " fulldamagealtadd", fullDamageAltAdd_ft,
    #            " damagealtaddmax", damageAltAddMax_ft,
    #            " damagevalue", damageValue," ", myNodeName );
    ...
    # limit amount of sinkage to damageAltMaxRate in one hit/loop--otherwise it
    # just goes down too fast, not realistic.  This is basically like the terminal
    ...
    if (damageAltAddPrev_ft != 0) {
      damageAltAddCurrent_ft = -abs((1 + 0.1*updateTime_s) * damageAltAddPrev_ft);
    } else {
      damageAltAddCurrent_ft=- abs(0.01*damageAltMaxPerCycle_ft);
    }

    # make sure this is not bigger than the max rate, if so only change
    # it by the max amount allowed per cycle
    if ( abs( damageAltAddCurrent_ft ) > abs(damageAltMaxPerCycle_ft ) ) {
      damageAltAddCurrent_ft =  damageAltMaxPerCycle_ft;
    }
  ...
  noPitch = 0;
  if (type=="aircraft" and ((damageValue > 0.8 and
                            (currAlt_ft <= objectsLowestAllowedAlt_ft and speed_kt > 20 ) or
                            (currAlt_ft <= objectsLowestAllowedAlt_ft-5)) or
  ...
  if ( damageValue > 0.8  ) {
    ...
    if (damageAltAddCurrent_ft ==0 and horizontalDistance_ft >0) {
      # forward
      pitchangle2_deg=0;
    }
    if (horizontalDistance_ft == 0 and damageAltAddCurrent_ft<0 ) {
      # straight down
      pitchangle2_deg=-90;
    }
    # Straight up won't happen here because we are (on purpose) forcing
    # the object down as we crash.  So we ignore the case.
    # if (horizontalDistance==0 and deltaAlt>0 ) {
    #   pitchangle2=90;
    #   straight up
    # }
    ...
    if (abs(pitchangle2_deg) > abs(pitchangle1_deg)) {
      pitchangle_deg=pitchangle2_deg;
    }

    # vert-speed prob
    # since we do this updateTime_ss per second the vertical speed in FPS (ideally) exactly equals damageAltAddCurrent*updateTime_s
    if ( type != "aircraft" ) {
      setprop(""~myNodeName~"/velocities/vertical-speed-fps",damageAltAddCurrent_ft * updateTime_s );
    }
  ...
  if (currTgtAlt_ft==nil) {
    currTgtAlt_ft=0;
  }

  if ((damageValue <= 0.8 ) or newTgtAlt_ft  < currTgtAlt_ft ) {
    # debprint("Bombable: setprop 1625");
    # target altitude--this is 10 feet or so in front of us for a ship or up to 1 minute in front for an aircraft
    setprop(""~myNodeName~"/controls/flight/target-alt", (newTgtAlt_ft ));
  ...
  if (lookingAheadAlt_ft > alt_ft ) {
    useAlt_ft = lookingAheadAlt_ft;
  } else {
    useAlt_ft=alt_ft;
  }
  calcAlt_ft = (useAlt_ft + alts.wheelsOnGroundAGL_ft + alts.targetAGL_ft + damageAltAddCumulative_ft + damageAltAddCurrent_ft);
  if (calcAlt_ft< objectsLowestAllowedAlt_ft) {
    calcAlt_ft=objectsLowestAllowedAlt_ft;
  }
  ...
  # above ground again.
    ...
    # debprint("correcting!", myNodeName, " ", toFrontAlt_ft,
    #           " ", currAlt_ft,
  ...
  if ( type == "aircraft" and  (damageValue > 0.8 )) {
    # debprint("Crashing! damageAltAdd & damageAltAddCurrent, damageAltAddPrev & damageAltAddMax,
    #                      damageAltMaxRate, damageAltMaxPerCycle ",damageAltAdd, " ",damageAltAddCurrent," ",
    #                      damageAltAddPrev, " ", damageAltAddMax, " ", myNodeName, " ",
    #                      damageAltMaxRate, " ", damageAltMaxPerCycle, " ", updateTime_s );
    ...
    if ( damageAltMaxPerCycle_ft <  damageAltAddCurrent_ft ) {
      ...
      if ( orientPitch > -10) {
        setprop(""~myNodeName~"/orientation/pitch-deg", orientPitch-1);
      }
    # put it down by the max allowed rate
      ...
      # it will roll/dodge as though under fire
      dodge(myNodeName);

    # closer to the ground than MaxPerCycle so, just put it right on the ground.  Oh yeah, also explode etc.
    } else {
  ...
  # debprint("alt = ", alt, " currAlt_ft = ", currAlt_ft,
  #           " deltaAlt= ", deltaAlt, " altAdjust= ", alts.wheelsOnGroundAGL_ft,
  #           " calcAlt_ft=", calcAlt_ft, "damageAltAddCurrent=", damageAltAddCurrent,
  #           " ", myNodeName);
}
#######################################################
# location-check loop, a timer function, every 15-16 seconds to check if
# the object has been relocated (this will happen if the object is set up
# as an AI ship or aircraft and FG is reset).  If so it restores the object
# to its position before the reset.
...
var location_loop = func(id, myNodeName) {
  ...
  prev_distance  = 0;
  # this will be set as previous/distance if we are initializing
  directDistance = 200;
...
var altClosestApproachCalc = func {
  ...
  # This formula calcs the closest distance the object would have passed from the exact
  # center of the target object, where 0 = a direct hit through the center of the object; on the XY plane
  ...
  # combine closest approach in XY and closest approach along the pitch angle
  # to get the overall point of closest approach
  var closestApproachOLDWAY_m = math.sqrt ( closestApproachXY_m * closestApproachXY_m + closestApproachPitch_m * closestApproachPitch_m);
  ...
  if (impactDistance_m<closestApproach_m) {
    debprint("#########CLOSEST APPROACH CALC ERROR########");
  }
}
...
var put_splash = func (nodeName, iLat_deg,iLon_deg, iAlt_m, ballisticMass_lb, impactTerrain="terrain", refinedSplash=0, myNodeName="" ) {
  ...
  if ((impactSplashPlaced==nil or impactSplashPlaced!=impactObjectLat_deg) and iLat_deg!=nil and iLon_deg!=nil and iAlt_m!=nil) {
    ...
    if (ballisticMass_lb>1.2 or
       (ballisticMass_lb <=1.2 and rand()<ballisticMass_lb/10) ) {
      settimer( func {
...
var getBallisticMass_lb = func (impactNodeName) {
  ...
  if (ballisticMass_slug != nil ) {
    ballisticMass_lb = ballisticMass_slug * 32.174049
  } else {
    ballisticMass_lb = 0.25;
    ...
    if (find("MK-81", impactType ) != -1 ) {
      ballisticMass_lb = 250;
    } elsif (find("MK-82", impactType ) != -1 ) {
      ballisticMass_lb = 500;
    } elsif (find("MK82", impactType ) != -1 ) {
      ballisticMass_lb = 500;
    } elsif (find("MK-83", impactType ) != -1 ) {
      ballisticMass_lb = 1000;
    } elsif (find("MK-84", impactType ) != -1 ) {
      ballisticMass_lb = 2000;
    } elsif (find("25 pound", impactType ) != -1 ) {
      ballisticMass_lb = 25;
    } elsif (find("5 pound", impactType ) != -1 ) {
      ballisticMass_lb = 5;
    } elsif (find("100 pound", impactType ) != -1 ) {
      ballisticMass_lb = 100;
    } elsif (find("150 pound", impactType ) != -1 ) {
      ballisticMass_lb = 150;
    } elsif (find("250 pound", impactType ) != -1 ) {
      ballisticMass_lb = 250;
    } elsif (find("500 pound", impactType ) != -1 ) {
      ballisticMass_lb = 500;
    } elsif (find("1000 pound", impactType ) != -1 ) {
      ballisticMass_lb = 1000;
    } elsif (find("2000 pound", impactType ) != -1 ) {
      ballisticMass_lb = 2000;
    } elsif (find("aim-9", impactType ) != -1 ) {
      ballisticMass_lb = 20.8;
    } elsif (find("AIM", impactType ) != -1 ) {
      ballisticMass_lb = 20.8;
    } elsif (find("WP-1", impactType ) != -1 ) {
      ballisticMass_lb = 23.9;
    } elsif (find("GAU-8", impactType ) != -1 ) {
      ballisticMass_lb = 0.9369635;
    } elsif (find("M-61", impactType ) != -1 ) {
      ballisticMass_lb = 0.2249;
    } elsif (find("M61", impactType ) != -1 ) {
      ballisticMass_lb = 0.2249;
    } elsif (find("LAU", impactType ) != -1 ) {
      # http://www.dtic.mil/dticasd/sbir/sbir041/srch/af276.pdf
      ballisticMass_lb = 86;
    } elsif (find("smoke", impactType ) != -1 ) {
      ballisticMass_lb = 0.0;
    } elsif (find(".50 BMG", impactType ) != -1 ) {
      ballisticMass_lb = 0.130072735;
    } elsif (find(".50", impactType ) != -1 ) {
      ballisticMass_lb = 0.130072735;
    } elsif (find("303", impactType ) != -1 ) {
      # http://en.wikipedia.org/wiki/Vickers_machine_gun
      ballisticMass_lb = 0.0264554715;
    } elsif (find("gun", impactType ) != -1 ) {
      ballisticMass_lb = 0.025;
    } elsif (find("bullet", impactType) != -1 ) {
      ballisticMass_lb = 0.0249122356;
    } elsif (find("tracer", impactType) != -1 ) {
      ballisticMass_lb = 0.0249122356;
    } elsif (find("round", impactType) != -1 ) {
      ballisticMass_lb = 0.9369635;
    } elsif (find("cannon", impactType ) != -1 ) {
      ballisticMass_lb = 0.282191696;
    } elsif (find("bomb", impactType ) != -1 ) {
      ballisticMass_lb = 250;
    } elsif (find("heavy-bomb", impactType ) != -1 ) {
      ballisticMass_lb = 750;
    } elsif (find("rocket", impactType ) != -1 ) {
      ballisticMass_lb = 50;
    } elsif (find("missile", impactType ) != -1 ) {
      ballisticMass_lb = 185;
    }
  }
...
var getImpactVelocity_mps = func (impactNodeName=nil,ballisticMass_lb=.25) {
  ...
  if (impactVelocity_mps == nil or impactVelocity_mps== 0) {
    if (ballisticMass_lb < 0.1) {
      impactVelocity_mps= 1200;
    } elsif (ballisticMass_lb < 0.5) {
      impactVelocity_mps= 900;
    } elsif (ballisticMass_lb < 2) {
      impactVelocity_mps= 500;
    } elsif (ballisticMass_lb < 50) {
      impactVelocity_mps= 250;
    } elsif (ballisticMass_lb < 500) {
      impactVelocity_mps= 150;
    } elsif (ballisticMass_lb < 2000) {
      impactVelocity_mps= 125;
    } else {
      impactVelocity_mps= 100;
    }
  }
...
var test_impact = func(changedNode, myNodeName) {
  ...
  if ( myNodeName == "" ) {
    ...
    if ( impactorHeading_deg == nil ) {
      impactorHeading_deg = rand() * 180 - 90;
    }

    impactorPitch_deg = getprop(""~impactNodeName~"/impact/pitch-deg");
    # if perchance this doesn't exist we'll just randomize it; it must be -90 to 90 or it wouldn't have hit.
    if ( impactorPitch_deg == nil ) {
      impactorPitch_deg = rand() * 180 - 90;
    }
  ...
  # total amoung of damage actually added as the result of the impact
  var damAdd        = 0;
  ...
  if ( !vitalDamageRadius_m ) {
    vitalDamageRadius_m = damageRadius_m/3;
  }
  ...
  # debprint("Bombable: Projected closest impact distance delta : ", closestApproachOLDWAY_m-closestApproach_m,
  #          "FG Impact Detection Point delta: ", impactDistance_m - cartesianDistance(deltaX_m,deltaY_m,deltaAlt_m),
  #          " ballisticMass_lb=", ballisticMass_lb);

  # AIManager.cxx it is 50 ft for aircraft & multiplayer;extra 5 m is fudge factor
  # var tgt_ht_m     = 50/.3042 + 5;
  # AIManager.cxx it is 100 ft for aircraft & multiplayer; extra 5 m is fudge factor
  # var tgt_length_m = 100/.3024 + 5;
  ...
  if ( closestApproach_m < damageRadius_m ) {
    ...
    if ( closestApproach_m <= vitalDamageRadius_m ) {
      ...
      impactLikelihood = (damageRadius_m - closestApproach_m)/(damageRadius_m -vitalDamageRadius_m);
    }
    ...
    if ( closestApproach_m <= vitalDamageRadius_m ) {
      ...
      # possibility of causing a routine amount of damage
      outsideIDdamagePotential=impactLikelihood;

      # debprint("Bombable: Direct hit, "~ impactNodeName~ " on ", myNodeName,
      #           " Distance= ", closestApproach_m, " heightDiff= ", deltaAlt_m,
      #           " terrain=", impactTerrain, " radius=", damageRadius_m,
      #           " dP:", damagePotential, " oIdP:", outsideIDdamagePotential,
      #           " bM:", ballisticMass_lb);
    } else {
      ...
      # you go out.
      #
      # ^2 makes it a bit too difficult to get a hit/let's try ^1.5 instead
      outsideIDdamagePotential = math.pow(impactLikelihood, 1.5);

      # debprint("Bombable: Near hit, "~ impactNodeName~ " on ", myNodeName,
      #           " Distance= ", closestApproach_m, " heightDiff= ", deltaAlt_m,
      #           " terrain=", impactTerrain, " radius=", damageRadius_m,
      #           " dP ", damagePotential, " OIdP ", outsideIDdamagePotential,
      #           " vitalHitchance% ", damagePotential*vuls.damageVulnerability*easyModeProbability * ballisticMass_lb / 5);
    ...
    if ( ballisticMass_lb < 1.2 ) {
      ...
      if ( rand() < damagePotential*easyModeProbability ) {
        ...
        # debprint("Bombable: Direct Hit/Nonvital hit. ballisticMass:
        #           ", ballisticMass_lb," outsideIDDamPotent: ", outsideIDdamagePotential,
        #           " weaponDamageCapab:", weaponDamageCapability  );
      ...
      var damagePoss = 0.6 + ballisticMass_lb/250;
      ...
      if ( rand() < damagePotential*vuls.damageVulnerability*easyModeProbability * ballisticMass_lb / 5  ) {
        ...
        # if it hits a regular or less vital spot
    ...
    # debprint("Bombable: Damaging hit, "~ " Distance= ", closestApproach_m,
    #           "by ", impactNodeName~ " on ", myNodeName," terrain=", impactTerrain,
    #           " damageRadius=", damageRadius_m," weaponDamageCapability ", weaponDamageCapability,
    #           " damagePotential ", damagePotential, " OIdP ", outsideIDdamagePotential,
    #           " Par damage: ", weaponDamageCapability * vuls.damageVulnerability);
    ...
    if ( damageCaused > 0 ) {
      ...
      if ( myNodeName == "" ) {
        ...
        # in the direction from object location to impact closest approach point,
        # and (since <impactorDirection> is the unit vector and <crossProd>'s magnitude is the distance from
  ...
  # end, case of direct hit
    ...
    if ( myNodeName == "" ) {
      ...
      if ( (impactSplashPlaced == nil) or
           (impactSplashPlaced != iLat_deg) and
           (impactRefinedSplashPlaced == nil) or
           (impactRefinedSplashPlaced != iLat_deg) and
           (ballisticMass_lb > 1.2) ) {
        var crossProdObj_ImpX_m = impactorDirectionY*crossProdZ_m - impactorDirectionZ*crossProdY_m;
        var crossProdObj_ImpY_m = impactorDirectionZ*crossProdX_m - impactorDirectionX*crossProdZ_m;
        var crossProdObj_ImpZ_m = impactorDirectionX*crossProdY_m - impactorDirectionY*crossProdX_m;
    ...
    if ( ballisticMass_lb < 1.2 ) {
      ...
      if ( closestApproach_m <= 10 + damageRadius_m ) {
        damAdd=add_damage(0.1 * vuls.damageVulnerability * ballisticMass_lb / 10 * easyMode, myNodeName, "weapon", impactNodeName, ballisticMass_lb, iLat_deg, iLon_deg, iAlt_m);
        ...
        if ( rand() < damFactor ) {
          damAdd=add_damage(0.0002 * vuls.damageVulnerability * ballisticMass_lb/10* easyMode, myNodeName, "weapon", impactNodeName, ballisticMass_lb, iLat_deg, iLon_deg, iAlt_m);
        }
      }
      ...
      if ( closestApproach_m <= .75 + damageRadius_m) {
        ...
        if ( rand() < damFactor ) {
          damAdd = add_damage(0.0002 * vuls.damageVulnerability * ballisticMass_lb/50* easyMode, myNodeName, "weapon", impactNodeName, ballisticMass_lb, iLat_deg, iLon_deg, iAlt_m);
        }
      } else {
        ...
        if ( rand() < damFactor ) {
          damAdd = add_damage(0.0001 * vuls.damageVulnerability * ballisticMass_lb/350* easyMode, myNodeName, "weapon", impactNodeName, ballisticMass_lb, iLat_deg, iLon_deg, iAlt_m);
        }
      }
      ...
      if ( closestApproach_m <= 1.5 + damageRadius_m ) {
        ...
        if ( rand() < damFactor ) {
          damAdd = add_damage(0.0002 * vuls.damageVulnerability * ballisticMass_lb/200 * easyMode, myNodeName, "weapon", impactNodeName, ballisticMass_lb, iLat_deg, iLon_deg, iAlt_m);
        }
      } else {
        var damFactor = (100-closestApproach_m)/100;
        if ( damFactor < 0 ) {
      ...
      if ( closestApproach_m <= 2 + damageRadius_m ) {
        ...
        if ( rand() < damFactor ) {
          damAdd = add_damage(0.0002 * vuls.damageVulnerability * ballisticMass_lb/350* easyMode, myNodeName, "weapon", impactNodeName, ballisticMass_lb, iLat_deg, iLon_deg, iAlt_m);
        }
      } else {
  ...
  if ( (type != "multiplayer") and (myNodeName!="") ) {
    ...
    if ( (closestApproach_m < 500) and (rand() < skill/14) ) {
      dodge(myNodeName);
    } elsif ( (damAdd > 0) and (rand()< 0.20 - 2*skill/100) ) {
...
var speed_adjust = func (myNodeName, time_sec ) {
  ...
  if ( airspeed_kt <= 0 ) {
    # avoid the div by zero issue
    airspeed_kt = 0.000001;
  }
  ...
  if ( getprop(""~myNodeName~"/bombable/attack-inprogress") or getprop( ""~myNodeName~"/bombable/dodge-inprogress") ) {
    targetSpeed_kt = vels.attackSpeed_kt;
  } elsif ( stores.fuelLevel (myNodeName) < 0.2 ) {
  ...
  if ( targetSpeed_kt <= minSpeed_kt ) {
    targetSpeed_kt = minSpeed_kt+20;
  }
  if ( maxSpeed_kt <= targetSpeed_kt ) {
    maxSpeed_kt = targetSpeed_kt*1.5;
  }
  ...
  if ( math.abs(currRoll_deg) > sustainRollLimit_deg ) {
    if ( math.abs(currRoll_deg) > sustainRollLimit_deg + sustainRollLimitTransition_deg ) {
      ...
      targetSpeed_kt = (vels.attackSpeed_kt - vels.cruiseSpeed_kt ) * (currRoll_deg - sustainRollLimit_deg) + vels.cruiseSpeed_kt;
    }
  }
  ...
  if ( math.abs(sin_pitch) < 5/128 ) {
    ...
    if ( airspeed_kt<targetSpeed_kt ) {
      ...
      if ( calcspeed_kt<minSpeed_kt ) {
        calcspeed_kt = minSpeed_kt;
      }
      var fact = 1-(calcspeed_kt-minSpeed_kt)/(targetSpeed_kt-minSpeed_kt);
    } else {
      var calcspeed_kt = airspeed_kt;
      if ( calcspeed_kt>maxSpeed_kt ) {
        calcspeed_kt = maxSpeed_kt;
      }
    ...
    # velocity.
    # if (termVel_kt < minSpeed_kt) {
    #   termVel_kt = minSpeed_kt;
    # }
    ...
    if ( termVel_kt > maxSpeed_kt ) {
      termVel_kt = maxSpeed_kt;
    }
  ...
  if ( finalSpeed_kt < minSpeed_kt / 3 ) {
    finalSpeed_kt = minSpeed_kt/3;
  }
...
var speed_adjust_loop = func ( id, myNodeName, looptime_sec) {
  ...
  settimer( func {
    speed_adjust_loop (id, myNodeName, looptime_sec);
  }, looptime_sec);

  # debprint("weapons_loop starting");

  if ( !getprop(bomb_menu_pp~"ai-aircraft-attack-enabled") or !getprop(bomb_menu_pp~"bombable-enabled") ) {
    return;
  }
...
var do_acrobatic_loop_loop = func (id, myNodeName, loop_time=20, full_loop_steps=100, exit_steps=100, direction="up", rolldirenter = "cc", rolldirexit="ccw", vert_speed_add_kt=225, loop_count=0  ) {
  ...
  # we want to accelerate vertically by vert_speed_add over the first 1/4
  # of the loop; then back to 0 over the next 1/4 of the loop, then to -vert_speed_add
  ...
  loop_count += 1;
  if ( loop_count <= exit_steps ) {
    settimer(func {
      do_acrobatic_loop_loop(id, myNodeName, loop_time, full_loop_steps, exit_steps, direction, rolldirenter, rolldirexit,vert_speed_add_kt, loop_count);
    }, loop_time/full_loop_steps);
  }

  var curr_vertical_speed_fps         = getprop("" ~ myNodeName ~ "/velocities/vertical-speed-fps");
  var curr_acrobat_vertical_speed_fps = getprop("" ~ myNodeName ~ "/velocities/bombable-acrobatic-vertical-speed-fps");

  if ( (loop_count < full_loop_steps/4) or (loop_count >= full_loop_steps*3/4) ) {
    var localdir = 1;
  } else {
    var localdir=-1;
  }
  ...
  # if ( sgn * curr_acrobat_vertical_speed_fps >=  sgn * proposed_vertical_speed_fps) {
  #   setprop("" ~ myNodeName ~ "/velocities/vertical-speed-fps", proposed_vertical_speed_fps);
  # }
...
var do_acrobatic_loop = func (myNodeName, loop_time=20, full_loop_steps=100, exit_steps=100,  direction="up", rolldirenter = "cc", rolldirexit="ccw", vert_speed_add_kt=nil ) {
  ...
  if ( (vert_speed_add_kt == nil) or (vert_speed_add_kt <= 0) ) {
    ...
    # trying70%
    vert_speed_add_kt = 0.70*getprop(""~myNodeName~"/velocities/true-airspeed-kt");
  }
...
var choose_random_acrobatic = func (myNodeName) {
  ...
  var direction="up";
  if ( (altAGL_m > alts.minimumAGL_m + 100) and (rand() > 0.5) ) {
    direction = "down";
  }
...
#
var choose_attack_acrobatic = func (myNodeName, dist, myHeading_deg, targetHeading_deg,
                                    courseToTarget_deg, deltaHeading_deg, currSpeed_kt,
  ...
  if ( math.abs(deltaHeading_deg) >= 100 ) {
    ...
    if ( (math.abs(normdeg180(myHeading_deg-targetHeading_deg)) < 90) and (dist < currSpeed_kt * time / 3600 * nmiles2meters) ) {
      var steps = 85;
    } else {
    ...
    if ( (currAlt_m-targetAlt_m < 0) or (currAlt_m - currElev_m < alts.minimumAGL_m + 200) or (currSpeed_kt > 0.75 * vels.cruiseSpeed_kt) ) {
      var direction = "up";
    } else {
      var direction = "down";
    }
...
var rudder_roll_climb = func (myNodeName, degrees=15, alt_ft=-20, time=10, roll_limit_deg=85 ){
  ...
  if (type=="aircraft" and math.abs(degrees) > 0.1 ) {
    aircraftRoll (myNodeName, degrees, time, roll_limit_deg);
  }
  ...
  if (currAlt_ft + alt_ft < alts.minimumAGL_ft ) {
    alt_ft = alts.minimumAGL_ft;
  }
  if (currAlt_ft + alt_ft > alts.maximumAGL_ft ) {
    alt_ft = alts.maximumAGL_ft;
  }
...
var dodge = func(myNodeName) {
  ...
  if ( !getprop(bomb_menu_pp~"ai-aircraft-attack-enabled") or
       getprop("" ~ myNodeName~"/bombable/attributes/damage")==1 or
       !getprop(bomb_menu_pp~"bombable-enabled") ) {
    return;
  }
  ...
  if (math.abs(currRoll_deg) > 30) {
    dodgeAmount_deg = - math.sgn(currRoll_deg)* dodgeAmount_deg;
  } else if (rand() > evas.dodgeROverLPreference_percent/100) {
    dodgeAmount_deg = -dodgeAmount_deg;
  }
  ...
  if (type=="aircraft") {
    ...
    if (dodgeDelay_remainder_sec<0) {
      dodgeDelay_remainder_sec = 0.1;
    }
    ...
    if (currSpeed_kt>2*vels.minSpeed_kt and
        currSpeed_kt>.75*vels.cruiseSpeed_kt and
        rand()< skill/7 and skill>=3 and
        dims.length_m < 22 and dims.width_m < 18 ) {
    ...
    if ( dodgeAltAmount_ft > 0 ) {
      dodgeVertSpeed_fps = math.abs ( evas.dodgeVertSpeedClimb_fps * dodgeAltAmount_ft/evas.dodgeAltMax_ft);
    }
    if ( dodgeAltAmount_ft < 0 ) {
      dodgeVertSpeed_fps = - math.abs ( evas.dodgeVertSpeedDive_fps * dodgeAltAmount_ft/evas.dodgeAltMin_ft );
    }

    # velocities/vertical-speed-fps seems to be fps * 1000 for some reason?
    # At least, approximately, 300,000 seems to be about 300 fps climb, for instance.
    # and we reduce the amount of climb/dive possible depending on the current
    # roll angle (can't climb/dive rapidly if rolled to 90 degrees . . . )
    ...
    #   setprop("" ~ myNodeName ~ "/velocities/vertical-speed-fps", dodgeVertSpeed_fps);
    # }
    ...
    settimer( func {
      aircraftRoll (myNodeName, dodgeAmount_deg, dodgeDelay_remainder_sec, evas.dodgeMax_deg);
    }, rollTime_sec);
  ...
  # other types besides aircraft
    ...
    # deduct the amount of fuel from the tank, for this dodge
    stores.reduceFuel (myNodeName, 2*dodgeDelay );
    settimer( func {
      setprop(""~myNodeName~"/bombable/dodge-inprogress", 0);
...
var getCallSign = func ( myNodeName ) {
  ...
  #AI or MP objects
    ...
    if (callsign==nil or callsign=="") {
      callsign=getprop(""~myNodeName~"/name");
    }
    if (callsign==nil or callsign=="") {
      node=props.globals.getNode(myNodeName);
...
var mp_update_damage = func (myNodeName="", damageRise=0, damageTotal=0, smokeStart=0, fireStart=0, callsign="" ) {
  ...
  # if ( myNodeName == "" ) {
  #   debprint("Bombable: Updating main aircraft 2328");
  # }
  ...
  if ( (damageValue >= 1) and (damageRise > 0) ) {
    ...
    # turn the big explosion off sorta quickly
    settimer(func {
      setprop("/bombable/fire-particles/smoke-startsize", smokeStartsize);
    }, 2.5);
    ...
    if ( (explosiveMass_kg == nil) or (explosiveMass_kg == 0) ) {
...
var mainAC_add_damage = func (damageRise=0, damageTotal=0, source="", message="") {
  ...
  if (damageIncrease>0)  {
    ...
    if (damageValue < .01) {
      msg = sprintf( addMsg1 ~ " Damage added %1.2f%% - Total damage %1.0f%%", damageIncrease*100 , damageValue*100 );
    } elsif (damageValue < .1) {
      msg = sprintf( addMsg1 ~ " Damage added %1.1f%% - Total damage %1.0f%%", damageIncrease*100 , damageValue*100);
    } elsif (damageValue < 1) {
      msg= sprintf( addMsg1 ~ " Damage added %1.0f%% - Total damage %1.0f%%", damageIncrease*100, damageValue*100);
    } else {
      msg= sprintf( "======== " ~ addMsg2 ~ " Damage added %1.0f%% - Total damage %1.0f%% ========", damageIncrease*100, damageValue*100 );
    }
  ...
  if (damageEngineSmokeStarted == nil ) {
    damageEngineSmokeStarted = 0;
  }
  ...
  if (((damageValue >= 1 - vuls.fireVulnerability_percent/100 and
        damageIncrease > 0 and !fireStarted) and
        (source=="weapons" or source=="crash" ) ) or
        (source=="crash" )) {
    ...
    var fp = "";
    if (vuls.explosiveMass_kg < 1000 ) {
      fp="AI/Aircraft/Fire-Particles/fire-particles-very-small.xml";
    } elsif (vuls.explosiveMass_kg > 5000 ) {
      fp="AI/Aircraft/Fire-Particles/fire-particles-small.xml";
    } elsif (vuls.explosiveMass_kg > 50000 ) {
      fp="AI/Aircraft/Fire-Particles/fire-particles-large.xml";
    } else {
      fp="AI/Aircraft/Fire-Particles/fire-particles.xml";
    }
  ...
  if (damageValue >= 1 and (prevDamageValue < 1 or
      getprop("/controls/engines/engine[0]/magnetos") > 0 or
      getprop("/controls/engines/engine[0]/throttle") > 0 )) {
    ...
    if (source=="weapons" or source=="crash" ) {
      startFire("");
    }
...
var fireAIWeapon = func (time_sec=1, myNodeName="") {
  ...
  settimer( func {
    fireAIWeapon_stop(loopid,myNodeName);
  }, time_sec);
}
...
var vertAngle_deg = func (geocoord1, geocoord2) {
  ...
  if ( dist == 0 ) {
    ...
    return math.asin((geocoord2.alt() - geocoord1.alt())/dist ) * R2D;
  }
}
...
var checkAim = func (myNodeName1="", myNodeName2="", targetSize_m=nil,
                     aiAimFudgeFactor=1, maxDistance_m=100, weaponAngle_deg=nil,
                     weaponOffset_m=nil, damageValue=0 ) {
  ...
  # if (myNodeName1=="/environment" or myNodeName1=="environment") {
  #   myNodeName1="";
  # }
  # if (myNodeName2=="/environment" or myNodeName2=="environment") {
  #   myNodeName2="";
  # }
  ...
  if ( targetSize_m==nil or targetSize_m.horz<=0 or targetSize_m.vert <=0 or maxDistance_m <= 0) {
    return 0;
  }
  ...
  if (distance_m < attributes[myNodeName1].dimensions.crashRadius_m) {
    ...
    if (vDamRad_m>=damRad_m) {
      vDamRad_m = damRad_m*0.95;
    }
  ...
  # as the object gets closer we can expand the degrees of a hit to be bigger;
  # at maxDistance it is X degrees but if 1/2 maxDistance, 2X degrees, etc
  # var factor = maxDistance_m/distance_m;

  if (myNodeName1=="") {
    myHeading_deg=getprop(""~myNodeName1~"/orientation/heading-deg");
  } else {
    myHeading_deg=getprop(""~myNodeName1~"/orientation/true-heading-deg");
  }
  ...
  # ranges 0 to 1, 1 being direct hit
  return result;
}
...
var weapons_loop = func (id, myNodeName1="", myNodeName2="", targetSize_m=nil) {
  ...
  var loopLength = 0.5;
  settimer( func {
    weapons_loop (id, myNodeName1, myNodeName2, targetSize_m );
  }, loopLength * (1 + rand()/8));

  # debprint("weapons_loop starting");

  if ( !getprop( bomb_menu_pp~"ai-aircraft-weapons-enabled") or !getprop(bomb_menu_pp~"bombable-enabled") ) {
    return;
  }

  # if no weapons set up for this Object then just return
  if ( !getprop(""~myNodeName1~"/bombable/initializers/weapons-initialized") ) {
    return;
  }
  ...
  if ( (aiAimFudgeFactor == nil) or (aiAimFudgeFactor == 0) ) {
    aiAimFudgeFactor = 11.5;
  }
  ...
  foreach (elem;keys (weaps) ) {
    ...
    myNodeName1, " ", weaps[elem].name, " ", elem, " accuracy ", round(result * 100 ),"%");
    ...
    if (r < result) {
      ...
      mainAC_add_damage ( damageAdd, 0, "weapons", "Hit from " ~ ai_callsign ~ " - " ~ weaponName ~"!");
...
stores.reduceWeaponsCount = func (myNodeName, elem, time_sec) {
  ...
  # Number of seconds worth of ammo firing the weapon has
...
stores.reduceFuel = func (myNodeName, time_sec) {
  ...
  # Number of seconds worth of combat time the AC has in fuel reserves.
  # TODO: This should be set per aircraft
  var fuel_seconds = 600;
...
stores.fillFuel = func (myNodeName,amount=1){
  ...
  if ( ! contains( attributes, myNodeName) or ! contains( attributes[myNodeName], "stores") ) {
    return;
  }
...
stores.checkWeaponsReadiness = func (myNodeName, elem=nil) {
  ...
  if (elem != nil ) {
    if (stos.weapons[elem]!=nil and stos.weapons[elem] == 0 ) {
      return 0;
    } else {
      return 1;
    }
...
stores.checkAttackReadiness = func (myNodeName) {
  ...
  if (stos["fuel"]!=nil and stos.fuel < .2) {
    ret=0;
  }
  msg ~= " fuel:"~ stos.fuel;
  var damage = getprop("" ~ myNodeName ~ "/bombable/attributes/damage");
  if (damage!=nil and damage > .8) {
    ret=0;
  }
  msg ~=" damage:"~ damage;
  ...
  weapret = 0;
  foreach (elem;keys (weaps) ) {
    if (stos.weapons[elem]!=nil and stos.weapons[elem] > .2 ) {
      weapret=1;
    }
...
stores.revitalizeAttackReadiness = func (myNodeName,dist_m=1000000){
  ...
  if (dist_m > atts.maxDistance_m * .9 ) {
    ...
    deleteFire(myNodeName);
    if (getprop(""~myNodeName~"/bombable/attributes/damage")<.25) {
      deleteSmoke("damagedengine", myNodeName);
    }
...
var any_aircraft_position = func (myNodeName) {
  ...
  # if (myNodeName=="") {
  #   debprint("Bombable: Updating main aircraft 2700");
  # }
...
var distAItoMainAircraft= func (myNodeName){
  ...
  return [ aiAircraftPosition.direct_distance_to(mainAircraftPosition), aiAircraftPosition.alt() - mainAircraftPosition.alt() ];
}
...
var attack_loop = func (id, myNodeName, looptime) {
  ...
  if (looptimealt!=nil and looptimealt > 0) {
    looptime = looptimealt;
  }
  ...
  settimer( func {
    attack_loop (id, myNodeName, looptime);
  }, looptimeActual);

  # we're going to say that dodging takes priority over attacking
  if ( getprop( ""~myNodeName~"/bombable/dodge-inprogress") ) {
    return;
  }
  ...
  if ( dist[0] < atts.minDistance_m ) {
    ...
    if (newAltLowerCutoff_m < 150) {
      newAltLowerCutoff_m = 200;
    }
    var newAltHigherCutoff_m = atts.altitudeHigherCutoff_m/4;
    if (newAltHigherCutoff_m < 150) {
      newAltHigherCutoff_m = 200;
    }

    if (math.abs ( deltaHeading_deg ) < atts.continueAttackAngle_deg or dist[1] < -newAltLowerCutoff_m or dist[1] > newAltHigherCutoff_m ) {
      continueAttack=1;
    }
  }
  ...
  if ( ! stores.checkAttackReadiness(myNodeName) ) {
     ...
     if (newMaxDist_m<atts.minDistance_m) {
       newMaxDist_m=atts.minDistance_m*1.5;
      }
  ...
  if (rand()<dist[0]/atts.maxDistance_m) {
    distanceFactor=0;
  }
  if (dist[1]<0) {
    if  (rand() < -dist[1]/atts.altitudeLowerCutoff_m) {
      distanceFactor=0;
    } elsif (rand() < dist[1]/atts.altitudeHigherCutoff_m) {
      distanceFactor=0;
    }
  }
  ...
  if ( ! (dist[0] < atts.maxDistance_m and
     ( dist[0] > atts.minDistance_m or continueAttack ) and
       dist[1] > -atts.altitudeLowerCutoff_m and
       dist[1] < atts.altitudeHigherCutoff_m and
       readinessAttack and
       ((attentionFactor and distanceFactor) or attack_inprogress ))) {
    ...
    if ( dist[0] > atts.maxDistance_m ) {
      stores.revitalizeAttackReadiness(myNodeName, dist[0]);
    }
  ...
  if ((attack_inprogress== nil or attack_inprogress==0) or (math.abs ( deltaHeading_deg ) >= 90) ) {
    ...
    if ( attackClimbDive_inprogress and
       ( attackClimbDive_targetAGL_m != nil and attackClimbDive_targetAGL_m > 0 and math.abs(attackClimbDive_targetAGL_m + elevTarget_m - currAlt_m) > 500 and
       ( rand()> 0.005 * skill))) {
      ...
      if (currSpeed_kt>2.2*vels.minSpeed_kt and rand() < (skill+8)/15) {
        if ( choose_attack_acrobatic(myNodeName, dist[0], myHeading_deg, targetHeading_deg, courseToTarget_deg, deltaHeading_deg, currSpeed_kt, skill, currAlt_m, targetAlt_m, elevTarget_m)) {
          return;
        }
      }
      ...
      if (deltaAlt_m > 0) {
        if ( deltaAlt_m > atts.divePower/6 ) {
          attackClimbDiveAddDirection = -1;
        }
      } else {
        if ( -deltaAlt_m > atts.climbPower/6 ) {
          attackClimbDiveAddDirection = 1;
        }
      }
      ...
      if (targetAGL_m < alts.minimumAGL_m) {
        targetAGL_m = alts.minimumAGL_m;
      }
      if (targetAGL_m > alts.maximumAGL_m) {
        targetAGL_m = alts.maximumAGL_m;
      }
...
var aircraftTurnToHeadingOld = func (myNodeName, heading_deg=0, roll_deg=30, turntime=15){
  ...
  while ( diff_heading_deg < 0 ) {
    diff_heading_deg += 360;
  }
  if (diff_heading_deg>180) {
    diff_heading_deg += - 360;
  }
...
var aircraftSetVertSpeed = func (myNodeName, dodgeAltAmount_ft, evasORatts="evas") {
  ...
  if ( dodgeAltAmount_ft > 150 ) {
    dodgeVertSpeed_fps = math.abs ( evas.dodgeVertSpeedClimb_fps );
  } elsif ( dodgeAltAmount_ft > 100 ) {
    dodgeVertSpeed_fps = math.abs ( evas.dodgeVertSpeedClimb_fps/divAmt*4 );
  } elsif ( dodgeAltAmount_ft > 75 ) {
    dodgeVertSpeed_fps = math.abs ( evas.dodgeVertSpeedClimb_fps/divAmt*3 );
  } elsif ( dodgeAltAmount_ft > 50 ) {
    dodgeVertSpeed_fps = math.abs ( evas.dodgeVertSpeedClimb_fps/divAmt*2 );
  } elsif ( dodgeAltAmount_ft > 25 ) {
    dodgeVertSpeed_fps = math.abs ( evas.dodgeVertSpeedClimb_fps/divAmt );
  } elsif ( dodgeAltAmount_ft > 12.5 ) {
    dodgeVertSpeed_fps = math.abs ( evas.dodgeVertSpeedClimb_fps/divAmt/2 );
  } elsif ( dodgeAltAmount_ft > 6 ) {
    dodgeVertSpeed_fps = math.abs ( evas.dodgeVertSpeedClimb_fps/divAmt/3 );
  } elsif ( dodgeAltAmount_ft > 0 ) {
    dodgeVertSpeed_fps = math.abs ( evas.dodgeVertSpeedClimb_fps/divAmt/5 );
  } elsif  ( dodgeAltAmount_ft < -150 ) {
    dodgeVertSpeed_fps = - math.abs ( evas.dodgeVertSpeedDive_fps);
  } elsif  ( dodgeAltAmount_ft < -100 ) {
    dodgeVertSpeed_fps = - math.abs ( evas.dodgeVertSpeedDive_fps/divAmt*4);
  } elsif  ( dodgeAltAmount_ft < -75 ) {
    dodgeVertSpeed_fps = - math.abs ( evas.dodgeVertSpeedDive_fps/divAmt*3);
  } elsif  ( dodgeAltAmount_ft < -50 ) {
    dodgeVertSpeed_fps = - math.abs ( evas.dodgeVertSpeedDive_fps/divAmt*2);
  } elsif  ( dodgeAltAmount_ft < -25 ) {
    dodgeVertSpeed_fps = - math.abs ( evas.dodgeVertSpeedDive_fps/divAmt);
  } elsif  ( dodgeAltAmount_ft < -12.5 ) {
    dodgeVertSpeed_fps = - math.abs ( evas.dodgeVertSpeedDive_fps/divAmt/2);
  } elsif  ( dodgeAltAmount_ft < -6 ) {
    dodgeVertSpeed_fps = - math.abs ( evas.dodgeVertSpeedDive_fps/divAmt/3);
  } elsif  ( dodgeAltAmount_ft < 0 )  {
    dodgeVertSpeed_fps = - math.abs ( evas.dodgeVertSpeedDive_fps/divAmt/5);
  }
  ...
  if (dodgeVertSpeed_fps!=0) {
    ...
    if (vertSpeedChange_fps>25) {
      vertSpeedChange_fps=25;
    }
    if (vertSpeedChange_fps<-25) {
      vertSpeedChange_fps=-25;
    }
    ...
    if (!stalling or vertSpeedChange_fps<0) {
      setprop("" ~ myNodeName ~ "/velocities/vertical-speed-fps", curr_vertical_speed_fps + vertSpeedChange_fps);
    }
...
var aircraftTurnToHeadingControl = func (myNodeName, id, targetdegrees=0, rolldegrees=45, targetAlt_m="none" ,  roll_limit_deg=85, correction=0 ) {
  ...
  if (math.abs(rolldegrees)>roll_limit_deg) {
    rolldegrees=roll_limit_deg*math.sgn(rolldegrees);
  }

  start_heading_deg=getprop(""~myNodeName~"/orientation/true-heading-deg");
  delta_heading_deg = targetdegrees-start_heading_deg;
  while ( delta_heading_deg < 0 ) {
    delta_heading_deg += 360;
  }
  if (delta_heading_deg>180) {
    delta_heading_deg += - 360;
  }
  ...
  if (rolltime<updateinterval_sec) {
    rolltime=updateinterval_sec;
  }
  ...
  # for this maneuver
  if (math.abs(targetRoll_deg) > math.abs(rolldegrees)) {
    targetRoll_deg = math.abs(rolldegrees) * math.sgn (targetRoll_deg);
  }

  # whereas roll_limit_deg is the absolute max for the aircraft
  if (math.abs(targetRoll_deg)>math.abs(roll_limit_deg)) {
    targetRoll_deg=math.abs(roll_limit_deg)*math.sgn(targetRoll_deg);
  }
  ...
  if (math.abs(targetRoll_deg)>rollMax_deg) {
    targetRoll_deg=rollMax_deg*math.sgn(targetRoll_deg);
  }

  # if (math.abs (currRoll_deg - targetRoll_deg)> 5) {
  #   debprint("Bombable: Changing roll: ",  -currRoll_deg + targetRoll_deg, " ", myNodeName, " 3085");
  # }
  ...
  if (targetAlt_m != "none") {
    ...
    if (targetAlt_m - currElev_m  < alts.minimumAGL_m ) {
      targetAlt_ft = (alts.minimumAGL_m + currElev_m)/feet2meters;
    }
    if (targetAlt_m - currElev_m  > alts.maximumAGL_m ) {
      targetAlt_ft = (alts.maximumAGL_m + currElev_m)/feet2meters;
    }
    ...
    if (dodgeAltAmount_ft > evas.dodgeAltMax_ft) {
      dodgeAltAmount_ft = evas.dodgeAltMax_ft;
    }
    if (dodgeAltAmount_ft < evas.dodgeAltMin_ft) {
      dodgeAltAmount_ft = evas.dodgeAltMin_ft;
    }
    dodgeVertSpeed_fps = 0;
...
var aircraftRollControl = func (myNodeName, id, rolldegrees=-90, rolltime=5, roll_limit_deg=85) {
  ...
  if (math.abs(rolldegrees)>= 90 ) {
    rolldegrees=88*math.sgn(rolldegrees);
  }
  ...
  if (targetRoll_deg >  roll_limit_deg ) {
    targetRoll_deg = roll_limit_deg;
  }
  if (targetRoll_deg < -roll_limit_deg) {
    targetRoll_deg =-roll_limit_deg;
  }
  ...
  if (math.abs(targetRoll_deg)>roll_limit_deg) {
    targetRoll_deg=roll_limit_deg*math.sgn(targetRoll_deg);
  }
  # rollMax_deg=getprop(""~myNodeName~"/bombable/attributes/attacks/rollMax_deg");
  ...
  if (math.abs(targetRoll_deg)>rollMax_deg) {
    targetRoll_deg=rollMax_deg*math.sgn(targetRoll_deg);
 	...
 	# if (math.abs (currRoll_deg - targetRoll_deg)> 5) {
 	#   debprint("Bombable: Changing roll: ", currRoll_deg - targetRoll_deg, " ", myNodeName, " 3238");
 	# }
  ...
  if ( rollTimeElapsed < rolltime ) {
    ...
    }, updateinterval_sec, roll_limit_deg );
  } else {
    setprop(""~myNodeName~ "/orientation/rollTimeElapsed", 0);
...
var aircraftRoll = func (myNodeName, rolldegrees=-60, rolltime=5, roll_limit_deg=85) {
  ...
  # turn it off after 10 seconds
  # settimer(func {
  #   removelistener(crashListener); crashListener=0;
  # }, 10);
}
...
var aircraftCrashControl = func (myNodeName) {
  ...
  if (getprop(""~myNodeName~"/bombable/attributes/damage")<1 ) {
    return;
  }
  ...
  delta_ft = 5.87*elapsed/(elapsed+5);
...
var aircraftCrash = func (myNodeName) {
  ...
  # if (myNodeName=="") {
  #   debprint("Bombable: Updating main aircraft 3154");
  # }
...
var variable_safe = func(str) {
  ...
  if (size(str)>0 and !string.isalpha(str[1])) {
    # make sure we always start with alpha char OR _
    s="_";
  }
...
var un_variable_safe = func(str) {
  ...
  for (var i = 0; i < size(str); i += 1) {
    if (str[i]==`_` and i !=1 )	{
      # ignore initial _, that is only to make a numeric value start with _ so it is a legal variable name
      s~=" ";
    } else {
      s ~= chr(str[i]);
    }
...
var insertionSort = func (a=nil, f=nil ) {
  ...
  if (f==nil) f = func(a, b) {
    if (num(a)==nil) {
      ...
      var acomp=sprintf("%f", a);
    }
    if (num(b)==nil) {
      var bcomp = b
    } else {
      var bcomp=sprintf("%f", b);
    }
    cmp (acomp,bcomp);
  }
...
records.record_impact = func ( myNodeName="", damageRise=0, damageIncrease=0, damageValue=0, impactNodeName=nil, ballisticMass_lb=nil, lat_deg=nil, lon_deg=nil, alt_m=nil ) {
  ...
  if (damageRise > 0 ) {
    me.impactTotals.Overall.Damaging_Impacts+=1;
  } else {
    me.impactTotals.Overall.Total_Impacts+=1;
  }
  ...
  if ( ballisticMass_lb < 1) {
    ballCategory="Small arms";
  } elsif ( ballisticMass_lb <= 10) {
    ballCategory="1-10 pound ordinance";
  } elsif ( ballisticMass_lb <= 100) {
    ballCategory="11-100 pound ordinance";
  } elsif ( ballisticMass_lb <= 500) {
    ballCategory="101-500 pound ordinance";
  } elsif ( ballisticMass_lb <= 1000) {
    ballCategory="501-1000 pound ordinance";
  } elsif ( ballisticMass_lb > 1000) {
    ballCategory="Over 1000 pound ordinance";
  }
  ...
  for (var count=0; count<size (items); count+=1  ) {
    ...
    if (item==ballCategory) {
      category="Ammo_Categories";
    }
    ...
    if ( damageRise > 0 ) {
      currHash[i]["Damaging_Impacts"]+=1;
    } elsif (currHash[i]["Total_Impacts"]!=nil) {
      currHash[i]["Total_Impacts"]+=1;
    }
...
records.sort_keys = func {
  ...
  # if ( hash[a]["sort"] < hash[b]["sort"]) {
  #   -1;
  # } elsif (hash[a]["sort"] > hash[b]["sort"]) {
  #    1;
  # } else {
  #    0;
  # }
  ...
  me.sortkey = insertionSort(k, func(a, b) {
    ...
    # but if some of the values are numbers we have to convert them to string
    if (num(hash[a].sort)==nil) {
      var acomp=hash[a].sort
    } else {
      var acomp=sprintf("%f", hash[a].sort);
    }
    if (num(hash[b].sort)==nil) {
      var bcomp=hash[b].sort
    } else {
      var bcomp=sprintf("%f", hash[b].sort);
    }
    cmp (acomp,bcomp);
  });
}
...
var add_damage = func(damageRise, myNodeName, damagetype="weapon", impactNodeName=nil, ballisticMass_lb=nil, lat_deg=nil, lon_deg=nil, alt_m=nil  ) {
  ...
  if ( damageValue>prevDamageValue ) {
    ...
    if (damageRise <.01) {
      damageRiseDisplay = sprintf ("%1.2f",damageRise*100);
    } elsif (damageRise <.1) {
      damageRiseDisplay = sprintf ("%1.1f",damageRise*100);
    }
    ...
    if (damagetype == "weapon" or damageRise >4 or rand()<.05) {
  ...
  # if ( onGround) {
  #   if (spds.maxSpeedReduce_percent<20) {
  #     spds.maxSpeedReduce_percent=20;
  #   }
  #   minSpeed=0;
  #   onGround=1;
  # } else {
  ...
  # spds.maxSpeedReduce_percent is a percentage
  maxSpeedReduceProp = 1-spds.maxSpeedReduce_percent/100;
  speedReduce = 1-damageValue;
  if (speedReduce < maxSpeedReduceProp) {
    speedReduce=maxSpeedReduceProp;
  }
  ...
  if (type=="aircraft")  {
    ...
    if (true_spd > minSpeed) {
      setprop(""~myNodeName~"/velocities/true-airspeed-kt", true_spd * speedReduce);
    }
  }
...
var checkRange = func (v=nil, low=nil, high=nil, default=1) {
  ...
  if ( low != nil and v < low  ) {
    v = low;
  }
  if ( high != nil and v > high ) {
    v = high;
  }
  return v;
}
...
var checkRangeHash = func (b=nil, v=nil, low=nil, high=nil, default=1) {
  if (contains(b, v)) {
    return checkRange (b[v],low, high, default);
  } else {
    return default;
  }
}
...
var initialize_func = func ( b ) {
  ...
  # joysticks & the like
  var init_allowed = 0;
  if (find("/ai/models/", b.objectNodeName ) != -1 ) {
    init_allowed=1;
  }
  if (find("/multiplayer/", b.objectNodeName ) != -1 ) {
    init_allowed=1;
  }
  ...
  if (contains(b, "altitudes") and typeof (b.altitudes) == "hash") {
    ...
    if (b.altitudes.crashedAGL_m == 0 ) {
      b.altitudes.crashedAGL_m      = -0.001;
    }

    # this is how ground_loop knows to initialize the alititude on its first call
    b.altitudes.initialized          = 0;
  ...
  if (contains(b, "evasions") and typeof (b.evasions) == "hash") {
    ...
    if (b.evasions.dodgeROverLPreference_percent==nil) {
      b.evasions.dodgeROverLPreference_percent=50;
    }
  ...
  if (contains(b, "dimensions") and typeof (b.dimensions) == "hash") {
    ...
    if (!contains(b.dimensions, "damageRadius_m")) {
      b.dimensions.damageRadius_m=nil;
    }
    b.dimensions.damageRadius_m = checkRange ( b.dimensions.damageRadius_m, 0, nil, 6 );
    if (!contains(b.dimensions, "vitalDamageRadius_m")) {
      b.dimensions.vitalDamageRadius_m=nil;
    }
    b.dimensions.vitalDamageRadius_m = checkRange ( b.dimensions.vitalDamageRadius_m, 0, nil, 2.5 );
    if (!contains(b.dimensions, "crashRadius_m")) {
      b.dimensions.crashRadius_m=nil;
    }
  ...
  if (contains(b, "velocities") and typeof (b.velocities) == "hash") {
    ...
    if (contains(b.velocities, "diveTerminalVelocities") and typeof (b.velocities.diveTerminalVelocities) == "hash") {
      ...
      if (count > 0) {
        ...
        # average of Camel & Zero values, so a good typical value
        b.velocities.diveTerminalVelocityFactor = 700;
      }
    }
    ...
    if (contains(b.velocities, "climbTerminalVelocities") and typeof (b.velocities.climbTerminalVelocities) == "hash") {
      ...
      if (count > 0) {
        ...
        # average of Camel & Zero values, so a good typical value
        b.velocities.climbTerminalVelocityFactor = 750;
      }
    }
  }
  # #damage sanity checking
  if (contains(b, "vulnerabilities") and typeof (b.vulnerabilities) == "hash") {
    if (b.vulnerabilities.damageVulnerability<=0) {
      b.vulnerabilities.damageVulnerability = 1;
    }
  ...
  if (contains(b, "attacks") and typeof (b.attacks) == "hash") {
    ...
    if (b.attacks.minDistance_m < 0) {
      b.attacks.maxDistance_m = 100;
    }
    if (b.attacks.maxDistance_m < b.attacks.minDistance_m ) {
      b.attacks.maxDistance_m = 2*b.attacks.minDistance_m;
    }
    if (b.attacks.rollMin_deg == nil ) {
      b.attacks.rollMin_deg = 30;
    }
    if (b.attacks.rollMin_deg < 0) {
      b.attacks.rollMin_deg = 100;
    }
    if (b.attacks.rollMax_deg == nil ) {
      b.attacks.rollMax_deg = 80;
    }
    if (b.attacks.rollMax_deg < b.attacks.rollMax_deg) {
      b.attacks.rollMax_deg = b.attacks.rollMin_deg + 30;
    }
    b.attacks.rollRateMax_degpersec = checkRangeHash ( b.attacks, "rollRateMax_degpersec", 1, 720, 45 );

    if (b.attacks.climbPower == nil ) {
      b.attacks.climbPower = 2000;
    }
    if (b.attacks.climbPower < 0) {
      b.attacks.climbPower = 2000;
    }
    if (b.attacks.divePower == nil ) {
      b.attacks.divePower = 4000;
    }
    if (b.attacks.divePower < 0) {
      b.attacks.divePower = 4000;
    }
    if (b.attacks.attackCheckTime_sec == nil ) {
      b.attacks.attackCheckTime_sec = 15;
    }
    if (b.attacks.attackCheckTime_sec < 0.1) {
      b.attacks.attackCheckTime_sec = 0.1;
    }
    if (b.attacks.attackCheckTimeEngaged_sec == nil ) {
      b.attacks.attackCheckTimeEngaged_sec = 1.25;
    }
    if (b.attacks.attackCheckTimeEngaged_sec < 0.1) {
      b.attacks.attackCheckTimeEngaged_sec = 0.1;
    }
  }
  ...
  if (contains(b, "weapons") and typeof (b.weapons) == "hash") {
    ...
    foreach (elem ; keys(b.weapons)) {
      n += 1;
      if (b.weapons[elem].name == nil ) {
        b.weapons[elem].name = "Weapon " ~ n;
      }
      if (b.weapons[elem].maxDamage_percent == nil ) {
        b.weapons[elem].maxDamage_percent = 5;
      }
      if (b.weapons[elem].maxDamage_percent < 0) {
        b.weapons[elem].maxDamage_percent = 0;
      }
      if (b.weapons[elem].maxDamage_percent > 100) {
        b.weapons[elem].maxDamage_percent = 100;
      }
      if (b.weapons[elem].maxDamageDistance_m == nil ) {
        b.weapons[elem].maxDamageDistance_m = 500;
      }
      if (b.weapons[elem].maxDamage_percent <= 0) {
        b.weapons[elem].maxDamageDistance_m = 1;
      }
      if (b.weapons[elem].weaponAngle_deg.heading == nil ) {
        b.weapons[elem].weaponAngle_deg.heading = 0;
      }
      if (b.weapons[elem].weaponAngle_deg.elevation == nil ) {
        b.weapons[elem].weaponAngle_deg.elevation = 0;
      }
      if (b.weapons[elem].weaponOffset_m.x == nil ) {
        b.weapons[elem].weaponOffset_m.x = 0;
      }
      if (b.weapons[elem].weaponOffset_m.y == nil ) {
        b.weapons[elem].weaponOffset_m.y = 0;
      }
      if (b.weapons[elem].weaponOffset_m.z == nil ) {
        b.weapons[elem].weaponOffset_m.z = 0;
      }
      ...
      if (b.weapons[elem].weaponSize_m.start == nil or b.weapons[elem].weaponSize_m.start <=0 ) {
        b.weapons[elem].weaponSize_m.start=0.07;
      }
      if (b.weapons[elem].weaponSize_m.end == nil or b.weapons[elem].weaponSize_m.end <=0 ) {
        b.weapons[elem].weaponSize_m.end = 0.05;
      }
    }
  }
...
var update_m_per_deg_latlon_loop = func (id) {
  ...
  settimer( func {
    update_m_per_deg_latlon_loop(id);
  }, 63.2345);
...
var bombable_init_func = func(myNodeName) {
  ...
  # joysticks & the like
  var init_allowed = 0;
  if ( find("/ai/models/", myNodeName ) != -1 ) {
    init_allowed = 1;
  }
  if ( find("/multiplayer/", myNodeName ) != -1 ) {
    init_allowed = 1;
  }
...
var ground_init = func (myNodeName="") {
  ...
  settimer(func {
    bombable.ground_init_func(myNodeName);
  }, 45 + rand(),1);
}
...
var location_init = func (myNodeName="") {
  ...
  settimer(func {
    bombable.location_init_func(myNodeName);
  }, 50 + rand(),1);
}
...
var location_init_func = func(myNodeName) {
  ...
  settimer(func {
    location_loop(loopid, myNodeName);
  }, 15.15 + rand());
...
var attack_init = func (myNodeName="") {
  ...
  settimer(func {
    bombable.attack_init_func(myNodeName);
  }, 55 + rand(),1 );
}
...
var attack_init_func = func(myNodeName) {
  ...
  if ( (attackCheckTime == nil) or (attackCheckTime < 0.5) ) {
    attackCheckTime = 0.5;
  }
  ...
  settimer(func {
    attack_loop(loopid, myNodeName, attackCheckTime);
  }, attackCheckTime + rand());
  ...
  if ( type == "aircraft" ) {
    ...
    settimer(func {
      speed_adjust_loop ( saloopid, myNodeName, .3 + rand()/30);
    }, 12+rand());
  }
...
var weaponsOrientationPositionUpdate_loop = func (id, myNodeName) {
  ...
  settimer(func {
    weaponsOrientationPositionUpdate_loop (id, myNodeName);
  }, .16 + rand()/50);

  # no need to do this if any of these are turned off
  # though we may update weapons_loop to rely on these numbers as well
  if ( ! getprop("/bombable/fire-particles/ai-weapon-firing") or
       ! getprop( trigger1_pp~"ai-weapon-fire-visual"~trigger2_pp) or
       ! getprop(bomb_menu_pp~"bombable-enabled") ) {
    return;
  }
  ...
  foreach (elem;keys (weaps) ) {
    ...
    setprop(myNodeName ~ "/" ~elem~"/orientation/pitch-deg", getprop(myNodeName~"/orientation/pitch-deg")+weaps[elem].weaponAngle_deg.elevation);

    setprop(myNodeName ~ "/" ~elem~"/orientation/true-heading-deg", getprop(myNodeName~"/orientation/true-heading-deg")+ weaps[elem].weaponAngle_deg.heading);

    setprop(myNodeName ~ "/" ~elem~"/position/altitude-ft", getprop(myNodeName~"/position/altitude-ft")+weaps[elem].weaponOffset_m.z*.3048);

    # todo: add the x & y offsets; they'll have to be rotated and then converted to lat/lon and that's going to be slow . . .
    setprop(myNodeName ~ "/" ~elem~"/position/latitude-deg", getprop(myNodeName~"/position/latitude-deg") );

    # todo: add the x & y offsets
    setprop(myNodeName ~ "/" ~elem~"/position/longitude-deg", getprop(myNodeName~"/position/longitude-deg"));
  }
}
...
var weapons_init_func = func(myNodeName) {
  ...
  if ( !contains(attributes[myNodeName], "weapons") ) {
    debprint("no attributes.weapons, exiting");
    weapsSuccess = 0;
  } else {
    weaps = attributes[myNodeName].weapons;
  ...
  if ( weapsSuccess == 0 ) {
    # alternatively we could implement a fake/basic armament here
  ...
  settimer(func {
    weaponsOrientationPositionUpdate_loop(loopid, myNodeName);
  }, 3 +rand());

  foreach (elem;keys (weaps) ) {
    put_tied_weapon(myNodeName, elem, weaps[elem].weaponSize_m.start, weaps[elem].weaponSize_m.end, "AI/Aircraft/Fire-Particles/projectile-tracer.xml");
  }
  ...
  if ( type != "multiplayer" ) {
    ...
    # settimer(  func {
    #   weapons_loop (myNodeName, "", vertAngle_deg, horzAngle_deg, atts.maxDamageDistance_m, atts.maxDamage_percent);
    # }, 5);
    ...
    settimer( func {
      weapons_loop (loopid, myNodeName, "", mainAircraftSize_m);
    }, 5 + rand());
  }
...
var weapons_del = func(myNodeName) {
  ...
  debprint("Bombable: Effect *weapons* unloaded for "~ myNodeName~ " weapons loopid="~ loopid ~" and weaponsOrientation loopid="~loopid2);
}
...
#
var impactReporters= [
  # this is the FG default reporter
...
# delay between checking for mp messages in seconds
var mpTimeDelayReceive =.12409348;

# delay between sending messages.
var mpTimeDelaySend    =.25100234;
var mpsendqueue        = [];

# wait ~5 seconds before initial send
settimer(func {
  mpprocesssendqueue();
}, 5.2534241);
...
# wait 30 sec before first damage check because sometimes there is a very high transient g-force on initial startup
settimer(func {
  damageCheck ();
}, 60.11);
...
var bombableInit = func {
  ...
  settimer(func {
    update_m_per_deg_latlon_loop(loopid);
  }, 5.5435);
  ...
  if ( (attributesSet == nil) or !attributesSet ) {
    setAttributes ();
  }
  ...
  setlistener("/bombable/attributes/damage", func {
    ...
    settimer(func {
      mp_send_main_aircraft_damage_update (0);
    }, 4.36);
  });
  ...
  # if ( broadcast == nil ) {
  #   print ("Bombable: Error, mp_broadcast was not set up correctly");
  # } else {
  # };
  # test_msg();
}
